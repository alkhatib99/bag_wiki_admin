...
class SectionService {
  ...
  final String sectionsUrl = '${AppConstants.baseUrl}/api/sections';
  ...
  //   try {
  //     final response = await http.get(Uri.parse('$sectionsUrl/$id'))
  //         .timeout(const Duration(seconds: 10));
  ...
  Future<SectionModel> updateSection(SectionModel section) async {
    try {
      final response = await http.put(
        Uri.parse('$sectionsUrl/${section.id}'),
        headers: {'Content-Type': 'application/json'},
        body: json.encode(section.toJson()),
      ).timeout(const Duration(seconds: 15));
  ...
  Future<void> deleteSection(int id) async {
    try {
      final response = await http.delete(Uri.parse('$sectionsUrl/$id'))
          .timeout(const Duration(seconds: 10));
